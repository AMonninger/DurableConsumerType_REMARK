...
class DurableConsumerType_Latest(IndShockConsumerType):
    ...
    def updatenNrmGrid(self): #Grid of Normalized Durable Stock
        if self.nNrmMin > 0:
            self.nNrmGrid = construct_grid(self.nNrmMin, self.nNrmMax, self.nNrmCount - 1, self.grid_type, self.NestFac)
            self.nNrmGrid = np.append(0.0, self.nNrmGrid)
        else:
    ...
    def updatemNrmGrid(self): # Grid of Normalized Market resouces if d\neq n
        if self.mNrmMin > 0:
            self.NrmGrid = construct_grid(self.mNrmMin, self.mNrmMax, self.mNrmCount - 1, self.grid_type, self.NestFac)
            self.mNrmGrid = np.append(0.0, self.mNrmGrid)
        else:
    ...
    def solve_terminal_solution(self):
        ...
        self.mNrmGrid_Total = lSuS_array.reshape((len(self.nNrmGridNow) * 2, 1))
        self.mNrmGrid_Total = np.sort(np.unique(self.mNrmGrid_Total))
        if self.alpha < 1:
            self.mNrmGrid_Total = np.append(self.mNrmGrid_Total, np.max([np.max(self.mNrmGrid_Total), self.mNrmMax]) + 1)
        else:
            self.mNrmGrid_Total = np.append(self.mNrmGrid_Total, 1)

        solution_shape = (len(self.nNrmGridNow), len(self.mNrmGrid_Total))
        ...
        for i_d in range(len(self.nNrmGridNow)):
            lS_idx = np.where(self.mNrmGrid_Total == lSuS_array[i_d][0])[0][0]
            uS_idx = np.where(self.mNrmGrid_Total == lSuS_array[i_d][1])[0][0]
            length = np.max([0, uS_idx - lS_idx - 1])
            d_aux = np.ones(len(self.mNrmGrid_Total)) * self.nNrmGridNow[i_d]
            x_aux = self.mNrmGrid_Total + (1 - self.adjC)*d_aux

            cFunc_array[i_d] = self.alpha * x_aux
            cFunc_array[i_d][lS_idx + 1: uS_idx] = self.mNrmGrid_Total[lS_idx + 1: uS_idx]

            dFunc_array[i_d] = (1 - self.alpha) * x_aux
            dFunc_array[i_d][lS_idx + 1: uS_idx] = d_aux[lS_idx + 1: uS_idx]

            inv_vFunc_array[i_d] = self.CRRAutility_inv(self.alpha * x_aux, (1 - self.alpha) * x_aux) #self.inv_vFuncAdj_terminal(d_aux[:lS_idx], self.mNrmGrid_Total[:lS_idx])
            inv_vFunc_array[i_d][lS_idx + 1: uS_idx] = self.CRRAutility_inv(self.mNrmGrid_Total[lS_idx + 1: uS_idx], d_aux[lS_idx + 1: uS_idx]) #self.inv_vFuncKeep_terminal(d_aux[lS_idx: uS_idx], self.mNrmGrid_Total[lS_idx: uS_idx])
        ...
        ####################################################################################################################
        # Interpolation
        self.cFunc_terminal = LinearFast(cFunc_array, [self.nNrmGridNow, self.mNrmGrid_Total])
        self.dFunc_terminal = LinearFast(dFunc_array, [self.nNrmGridNow, self.mNrmGrid_Total])
        self.exFunc_terminal = LinearFast(exFunc_array, [self.nNrmGridNow, self.mNrmGrid_Total])
        self.inv_vFunc_terminal = LinearFast(inv_vFunc_array, [self.nNrmGridNow, self.mNrmGrid_Total])
        self.vFunc_terminal = ValueFuncCRRA(self.cFunc_terminal, self.CRRA)
        self.adjusting_terminal = LinearFast(adjusting_array, [self.nNrmGridNow, self.mNrmGrid_Total])
...
class DurableConsumerSolution(MetricObject):
    ...
    """
    distance_criteria = ["vPFunc", "vPFuncKeep", "vPFuncAdj"]
    ...
    def __init__(
        ...
        self.dFunc = dFunc if dFunc is not None else NullFunc()
        ...
        self.exFunc = exFunc if exFunc is not None else NullFunc()
...
def solve_DurableConsumer(
        ...
        for i_d in range(len(nNrmGridNow)):
            ...
            qFunc_array[i_d] = (
                    ...
                    expected(vp_next_vPFunc, IncShkDstn, args=(
                        vPFuncNext, aNrmGridNow, nNrmGridNow[i_d],
                        CRRA, dDepr, Rfree, PermGroFac)))

            wFunc_array[i_d] = (
                    DiscFac * LivPrb * expected(vFunc_next, IncShkDstn, args=(
                utility_inv, inv_vFunc_next, aNrmGridNow, nNrmGridNow[i_d], CRRA, dDepr,
                Rfree, PermGroFac)))
    ...
    aNrm_temp = np.insert(aNrmGridNow, 0, BoroCnstNat)
    inv_wFunc = LinearFast(inv_wFunc_array, [nNrmGridNow, aNrm_temp])
    wFunc = ValueFuncCRRA(inv_wFunc, CRRA)

    # ix. Interpolate and make functions
    qFunc = LinearFast(qFunc_array, [nNrmGridNow, aNrmGridNow])

    # print(np.max(qFunc_array))
    ...
    if dDepr == 1:
        if alpha == 1:
            mNrmGridNow = mNrmGridKeeper # For adjuster problem
            vFuncmNrmGridNow = mNrmMinNow + aXtraGrid # For value function
            vFuncmNrmGridNow = np.insert(vFuncmNrmGridNow, 0, BoroCnstArt)
        else:
            mNrmGridNow = mNrmGridNow + BoroCnstNat

    if np.min(cFuncKeepUnc_array)<0:
        print('error: cFuncKeepUnc is negative')

    for i_d in range(len(nNrmGridNow)):
        if np.min(cFuncKeepUnc_array[i_d]) < 0:
            print(i_d)
    ...
    if dDepr == 1:
        shape = (len(nNrmGridNow), len(mNrmGridNow))
        inv_vFuncKeep_array = np.zeros(shape)
        for i_d in range(len(nNrmGridNow)):
            d_aux = np.ones(len(mNrmGridNow)) * nNrmGridNow[i_d]
            inv_vFuncKeep_array[i_d] = inv_vFuncKeepUnc(d_aux, mNrmGridNow)
        inv_vFuncKeep = inv_vFuncKeepUnc
        vFuncKeep = vFuncKeepUnc
    else:
    ...
    if alpha == 1: #Use keeper solution
        ...
        # Exogenous xNrmGrid
        if dDepr == 1:  # Use xNrmGridNow over the Unconstraint part (starts at natural borrowing constraint which is smaller than 0)
            xMin = 0
            xMax = mNrmGridNow[-1] - BoroCnstNat
        else:
            xMin = mNrmGridNow[0] + (1 - adjC) * nNrmGridNow[0]
            xMax = mNrmGridNow[-1] + (1 - adjC) * nNrmGridNow[-1]

        xNrmGridNow = construct_grid(xMin, xMax, len(xNrmGrid), grid_type, NestFac)
        if dDepr == 1:
            xNrmGridNow = xNrmGridNow + BoroCnstNat

        if AdjX == True:
            # Create empty container
            adjust_shape = (len(xNrmGridNow))
            dFuncAdj_array = np.zeros(adjust_shape)
            cFuncAdj_array = np.zeros(adjust_shape)
            exFuncAdj_array = np.zeros(adjust_shape)
            inv_vFuncAdj_array = np.zeros(adjust_shape)

            # Create a Boolean if the Borrowing Constraint is hit.
            BoroCnstdNrmBool = 0
            # loop over x states
            for i_x in range(len(xNrmGridNow)):
                if i_x == 0:
                    continue
                x = xNrmGridNow[i_x]
                ...
                if alpha == 1:
                    ...
                    if dDepr == 1:
                        cFuncAdj_array[i_x] = cFuncKeepUnc(dFuncAdj_array[i_x], m)
                    else:
                        cFuncAdj_array[i_x] = cFuncKeep(dFuncAdj_array[i_x], m)
                    exFuncAdj_array[i_x] = cFuncAdj_array[i_x]

                else:
                    x0 = dFuncAdj_array[i_x - 1] # Use previous optimal d as starting point
                    sol_opt = sp.optimize.minimize(lambda d: - inv_vFuncKeep(d, x - d), x0, method='nelder-mead',
                                        options={'fatol': 1e-15})

                    # c. optimal value
                    # Add Borrowing constraint
                    if BoroCnstdNrm > sol_opt.x:
                    ...
                    m = x - dFuncAdj_array[i_x] # Budget Constraint
                    if dDepr == 1:
                        cFuncAdj_array[i_x] = cFuncKeepUnc(dFuncAdj_array[i_x], m)
                    else:
                    ...
                    if m - cFuncAdj_array[i_x] <= BoroCnstNat:
                        print("Borrowing Constrained hit")
                inv_vFuncAdj_array[i_x] = inv_vFuncKeep(dFuncAdj_array[i_x], x - dFuncAdj_array[i_x])
            inv_vPFuncAdj_array = CRRAutilityP_inv(cFuncAdj_array, dFuncAdj_array)

            # if kink at last point (primary or secondary)
            if cFuncAdj_array[-1] < cFuncAdj_array[-2]:
                print('cFuncAdj becomes a decreasing function')

            # Create Functions
            cFuncAdj = LinearFast(cFuncAdj_array, [xNrmGridNow])
            dFuncAdj = LinearFast(dFuncAdj_array, [xNrmGridNow])
            exFuncAdj = LinearFast(exFuncAdj_array, [xNrmGridNow])
            inv_vPFuncAdj = LinearFast(inv_vPFuncAdj_array, [xNrmGridNow])
            inv_vFuncAdj = LinearFast(inv_vFuncAdj_array, [xNrmGridNow])
            vFuncAdj = ValueFuncCRRA(inv_vFuncAdj, CRRA)
            vPFuncAdj = MargValueFuncCRRA_dur(cFuncAdj, dFuncAdj, alpha, d_ubar,
                                        CRRA)
        else:
            # Create empty container
            adjust_shape = (len(nNrmGridNow), len(mNrmGridNow))
            dFuncAdj_array = np.zeros(adjust_shape)
            cFuncAdj_array = np.zeros(adjust_shape)
            exFuncAdj_array = np.zeros(adjust_shape)
            inv_vFuncAdj_array = np.zeros(adjust_shape)
            ...
            # Create a Boolean if the Borrowing Constraint is hit.
            BoroCnstdNrmBool = 0
            # loop over n and m states
            for i_d in range(len(nNrmGridNow)):
                for i_m in range(len(mNrmGridNow)):
                    if i_m == 0:
                        continue
                    # a. cash-on-hand
                    x = mNrmGridNow[i_m] + (1 - adjC) * nNrmGridNow[i_d]

                    # b. optimal choice of d
                    # special case if alpha = 1
                    if alpha == 1:
                        dFuncAdj_array[i_d, i_m] = 0
                        if dDepr == 1:
                            cFuncAdj_array[i_d, i_m] = cFuncKeepUnc(0, x)
                        else:
                            cFuncAdj_array[i_d, i_m] = cFuncKeep(0, x)
                        exFuncAdj_array[i_d, i_m] = cFuncAdj_array[i_d, i_m]

                    else:
                        x0 = dFuncAdj_array[i_d, i_m - 1] # Use previous optimal d as starting point
                        sol_opt = sp.optimize.minimize(lambda d: - inv_vFuncKeep(d, x - d), x0, method='nelder-mead',
                                            options={'fatol': 1e-15})
                        # c. optimal value
                        # Add Borrowing constraint
                        if BoroCnstdNrm > sol_opt.x:
                            dFuncAdj_array[i_d, i_m] = BoroCnstdNrm
                            BoroCnstdNrmBool = 1
                        else:
                            dFuncAdj_array[i_d, i_m] = sol_opt.x

                        m = x - dFuncAdj_array[i_d, i_m]  # Budget Constraint
                        if dDepr == 1:
                            cFuncAdj_array[i_d, i_m] = cFuncKeepUnc(dFuncAdj_array[i_d, i_m], m)  # Evaluate cFunc at x and m
                        else:
                            cFuncAdj_array[i_d, i_m] = cFuncKeep(dFuncAdj_array[i_d, i_m], m)  # Evaluate cFunc at x and m
                        exFuncAdj_array[i_d, i_m] = cFuncAdj_array[i_d, i_m] + dFuncAdj_array[i_d, i_m]

                        if m - cFuncAdj_array[i_d, i_m] <= BoroCnstNat:
                            print("Borrowing Constrained hit")
                    inv_vFuncAdj_array[i_d, i_m] = inv_vFuncKeep(dFuncAdj_array[i_d, i_m], x - dFuncAdj_array[i_d, i_m])
            inv_vPFuncAdj_array = CRRAutilityP_inv(cFuncAdj_array, dFuncAdj_array)

            # if kink at last point (primary or secondary)
            for i_d in range(len(nNrmGridNow)):
                if cFuncAdj_array[i_d, -1] < cFuncAdj_array[i_d, -2]:
                    print(i_d)
                    print('cFuncAdj becomes a decreasing function')

            # Create Functions
            cFuncAdj = LinearFast(cFuncAdj_array, [nNrmGridNow, mNrmGridNow])
            dFuncAdj = LinearFast(dFuncAdj_array, [nNrmGridNow, mNrmGridNow])
            exFuncAdj = LinearFast(exFuncAdj_array, [nNrmGridNow, mNrmGridNow])
            inv_vPFuncAdj = LinearFast(inv_vPFuncAdj_array, [nNrmGridNow, mNrmGridNow])
            inv_vFuncAdj = LinearFast(inv_vFuncAdj_array, [nNrmGridNow, mNrmGridNow])
            vFuncAdj = ValueFuncCRRA(inv_vFuncAdj, CRRA)
            vPFuncAdj = MargValueFuncCRRA_dur(cFuncAdj, dFuncAdj, alpha, d_ubar,
                                        CRRA)


        if np.min(cFuncAdj_array) < 0:
            print('error: cFuncAdjUnc is negative')
        if np.max(dFuncAdj_array) >= np.max(nNrmGridNow):
            print("nNrmMax is smaller than optimal adjuster solution. Increase nNrmMax!")
        ...
        if dDepr == 1:
            if AdjX == True:
                ...
                cFuncAdjUnc = cFuncAdj
                dFuncAdjUnc = dFuncAdj
                exFuncAdjUnc = exFuncAdj
                ...
                # Recalculate the Value Function (From Borrowing Constraint

            else:
                cFuncAdjCnst = LinearFast(np.array([[0.0, alpha],[alpha*(1 - adjC), alpha*(2 - adjC)]]), [np.array([0.0,1.0]), np.array([0.0,1.0])])
                dFuncAdjCnst = LinearFast(np.array([[0.0, (1 - alpha)],[(1 - alpha)*(1 - adjC),(1 - alpha)*(1 + (1 - adjC))]]), [np.array([0.0,1.0]), np.array([0.0,1.0])])
                exFuncAdjCnst = LinearFast(np.array([[0.0, 1.0],[1 - adjC, 2 - adjC]]), [np.array([0.0,1.0]), np.array([0.0,1.0])])

                cFuncAdjUnc = cFuncAdj
                dFuncAdjUnc = dFuncAdj
                exFuncAdjUnc = exFuncAdj
                cFuncAdj = LowerEnvelope2D(cFuncAdjUnc, cFuncAdjCnst, nan_bool=False)
                dFuncAdj = LowerEnvelope2D(dFuncAdjUnc, dFuncAdjCnst, nan_bool=False)
                exFuncAdj = LowerEnvelope2D(exFuncAdjUnc, exFuncAdjCnst, nan_bool=False)
    ...
    if alpha == 1: # Use keeper solution
        ...
        # Create inverse value function for adjuster in (n x m) dimension
        solution_shape = (len(nNrmGridNow), len(mNrmGridNow))
        if AdjX == True:
            ...
            inv_vFuncAdjNM_array = inv_vFuncAdj_array
        # Calculate (S,s) thresholds
        if adjC > 0:
            mNrmGrid_Total, lSuS_array = durable_adjusting_function_fast(nNrmGridNow, mNrmGridNow,
                                                                    inv_vFuncAdjNM_array, inv_vFuncKeep_array,
                                                                    tol)
            mNrmGrid_Total = np.sort(np.append(mNrmGrid_Total, np.max(mNrmGrid_Total)-0.0001))
            # Check if nan's:
            if np.isnan(lSuS_array).any():
                print('Nans detected')
        else: #If AdjC = 0: Always adjust
            mNrmGrid_Total = mNrmGridNow
            lSuS_array = np.ones(solution_shape) * mNrmGrid_Total[0]

        # Create arrays with optimal solution conditional on (S,s) bands
        # Create empty container
        shape = (len(nNrmGridNow), len(mNrmGrid_Total))
        ...
        for i_d in range(len(nNrmGridNow)):
            d_aux = np.ones(len(mNrmGrid_Total)) * nNrmGridNow[i_d]
            x_aux = mNrmGrid_Total + (1 - adjC) * d_aux

            if dDepr == 1:
                if AdjX == True:
                    cFuncAdj_NM_array[i_d] = cFuncAdjUnc(x_aux)
                    dFuncAdj_NM_array[i_d] = dFuncAdjUnc(x_aux)
                else:
                    cFuncAdj_NM_array[i_d] = cFuncAdjUnc(d_aux, mNrmGrid_Total)
                    dFuncAdj_NM_array[i_d] = dFuncAdjUnc(d_aux, mNrmGrid_Total)
                cFuncKeep_NM_array[i_d] = cFuncKeepUnc(d_aux, mNrmGrid_Total)
                dFuncKeep_NM_array[i_d] = dFuncKeepUnc(d_aux, mNrmGrid_Total)
            else:
                if AdjX == True:
                    cFuncAdj_NM_array[i_d] = cFuncAdj(x_aux)
                    dFuncAdj_NM_array[i_d] = dFuncAdj(x_aux)
                else:
                    cFuncAdj_NM_array[i_d] = cFuncAdj(d_aux, mNrmGrid_Total)
                    dFuncAdj_NM_array[i_d] = dFuncAdj(d_aux, mNrmGrid_Total)
                cFuncKeep_NM_array[i_d] = cFuncKeep(d_aux, mNrmGrid_Total)
                dFuncKeep_NM_array[i_d] = dFuncKeep(d_aux, mNrmGrid_Total)

            if AdjX == True:
                inv_vFuncAdj_NM_array[i_d] = inv_vFuncAdj(x_aux)
            else:
                inv_vFuncAdj_NM_array[i_d] = inv_vFuncAdj(d_aux, mNrmGrid_Total)

            inv_vFuncKeep_NM_array[i_d] = inv_vFuncKeep(d_aux, mNrmGrid_Total)

        cFunc_array, dFunc_array, exFunc_array, inv_vFunc_array, adjusting_array = durable_solution_function_fast(
            nNrmGridNow, mNrmGrid_Total, lSuS_array, adjusting_array,
        ...
        cFunc = LinearFast(cFunc_array, [nNrmGridNow, mNrmGrid_Total])
        dFunc = LinearFast(dFunc_array, [nNrmGridNow, mNrmGrid_Total])
        exFunc = LinearFast(exFunc_array, [nNrmGridNow, mNrmGrid_Total])
    ...
    if alpha < 1:
        ...
        inv_vPFunc = LinearFast(inv_vPFunc_array, [nNrmGridNow, mNrmGrid_Total])
        vPFunc = MargValueFuncCRRA_dur(cFunc, dFunc, alpha, d_ubar, CRRA) #LinearFast(vPFunc_array, [nNrmGridNow, mNrmGrid_Total])
        adjusting = LinearFast(adjusting_array, [nNrmGridNow, mNrmGrid_Total])
        inv_vFunc = LinearFast(inv_vFunc_array, [nNrmGrid, mNrmGrid_Total])
    ...
    if dDepr == 1:
        ...
        for i_d in range(len(nNrmGridNow)):
            ...
            if AdjX == True:
                ...
                inv_vFuncKeep_aux = inv_vFuncKeep(d_keep_aux, vFuncmNrmGridNow[1:])
                inv_vFuncAdj_aux = inv_vFuncAdj(d_keep_aux, vFuncmNrmGridNow[1:])
