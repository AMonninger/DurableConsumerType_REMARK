...
def solve_adj(t, sol, par):
    ...
    for i_p in prange(par.Np):
        ...
        for i_x in range(par.Nx):
            ...
            d[i_p, i_x] = golden_section_search.optimizer(
                obj_adj,
                d_low,
                d_high,
                args=(x, inv_v_keep[i_p], grid_n, grid_m),
                tol=par.tol,
            )

            # c. optimal value
            m = x - d[i_p, i_x]
            c[i_p, i_x] = linear_interp.interp_2d(
                par.grid_n, par.grid_m, c_keep[i_p], d[i_p, i_x], m
            )
