...
def value_of_choice(t, c, d, p, x, inv_v_keep, inv_v_adj, par):
    ...
    for ishock in range(par.Nshocks):
        ...
        # iv. update
        inv_v_plus_keep_now = linear_interp.interp_3d(
            par.grid_p,
            par.grid_n,
            par.grid_m,
            inv_v_keep[t + 1],
        ...
        inv_v_plus_adj_now = linear_interp.interp_2d(
            par.grid_p, par.grid_x, inv_v_adj[t + 1], p_plus, x_plus
        )
...
def obj_keep(c, t, n, p, m, inv_v_keep, inv_v_adj, par):  # max wrt. c
    ...
    return (
        -value_of_choice(t, c, d, p, x, inv_v_keep, inv_v_adj, par) + penalty
    )  # minimization
    ...
    for i_p in prange(par.Np):  # loop in parallel
        for i_n in range(par.Nn):
            ...
            for i_m in range(par.Nm):
                ...
                # c. optimal value
                v = -obj_keep(
                    c[i_p, i_n, i_m], t, n, p, m, sol.inv_v_keep, sol.inv_v_adj, par
                )
...
def solve_adj(t, sol, par):
    ...
    for i_p in prange(par.Np):
        ...
        for i_x in range(par.Nx):
            ...
            # b. optimal choice
            results = qe.optimize.nelder_mead(
                obj_adj,
                choices,
