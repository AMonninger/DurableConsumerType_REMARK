...
def _discrete(model, t, i_p):
    ...
    ax.set_title(
        f"optimal discrete choice ($t = {t}$, $p = {par.grid_p[i_p]:.2f}$)", pad=10
    )
...
def _adj(model, t, i_p):
    ...
    ax_v.set_title(
        f"neg. inverse $v^{{adj}}$ ($t = {t}$, $p = {par.grid_p[i_p]:.2f}$)", pad=10
    )
...
def _keep(model, t, i_p):
    ...
    # d. plot value function
    ax_v.plot_surface(
        n, m, sol.inv_v_keep[t, i_p, :, :], cmap=cm.viridis, edgecolor="none"
    )
    ax_v.set_title(
        f"neg. inverse $v^{{keep}}$ ($t = {t}$, $p = {par.grid_p[i_p]:.2f}$)", pad=10
    )
