# -*- coding: utf-8 -*-
"""
Created on Mon Feb 28 15:49:02 2022

Normalized Version without a persistent component.
@author: Adrian
"""
...
class DurableConsumptionModelClass_NrM(ModelClass):
    ...
    def create_grids(self):
        ...
        # Using Income Shock from HARK: START
        par.psi = np.array([0.8659664865888173, 0.8659664865888173, 0.8659664865888173,
       0.8659664865888173, 0.8659664865888173, 0.9436041054762399,
       0.9436041054762399, 0.9436041054762399, 0.9436041054762399,
       0.9436041054762399, 0.9951180154971063, 0.9951180154971063,
       0.9951180154971063, 0.9951180154971063, 0.9951180154971063,
       1.0495192553481525, 1.0495192553481525, 1.0495192553481525,
       1.0495192553481525, 1.0495192553481525, 1.1457921370896842,
       1.1457921370896842, 1.1457921370896842, 1.1457921370896842,
       1.1457921370896842])
        par.psi = np.transpose(par.psi)

        par.xi = np.array([0.8659664865888173, 0.9436041054762399, 0.9951180154971063,
       1.0495192553481525, 1.1457921370896842, 0.8659664865888173,
       0.9436041054762399, 0.9951180154971063, 1.0495192553481525,
       1.1457921370896842, 0.8659664865888173, 0.9436041054762399,
       0.9951180154971063, 1.0495192553481525, 1.1457921370896842,
       0.8659664865888173, 0.9436041054762399, 0.9951180154971063,
       1.0495192553481525, 1.1457921370896842, 0.8659664865888173,
       0.9436041054762399, 0.9951180154971063, 1.0495192553481525,
       1.1457921370896842])
        par.xi = np.transpose(par.xi)
        # END
    ...
    def solve_prep(self):
        ...
        # adj_shape = (par.T, par.Np, par.Nx)  # changed
        adj_shape = (par.T, par.Nx)  # changed
        ...
        # post_shape = (par.T - 1, par.Np, par.Nn, par.Na)  # changed
        post_shape = (par.T - 1, par.Nn, par.Na)  # changed
        ...
        # keep_shape = (0, 0, 0, 0, 0)  # changed
        keep_shape = (0, 0, 0, 0)  # changed
        ...
        # adj_full_shape = (0, 0, 0)  # changed
        adj_full_shape = (0, 0)  # changed
        ...
        # adj_part_shape = (0, 0, 0, 0)  # changed
        adj_part_shape = (0, 0, 0)  # changed
        ...
        # post_shape = (0, 0, 0, 0, 0)  # changed
        post_shape = (0, 0, 0, 0)  # changed
        ...
        for t in reversed(range(self.par.T)):
            ...
            with jit(self) as model:
                ...
                # ii. all other periods
                    ...
                    #                    post_decision_HARK2.post_decision(t,sol,par,compute_q=True)
                    post_decision_NrM.compute_wq(t, sol, par, compute_q=True)
                    ...
                    # SUBSTITUTE WITH DCEGM
                    negm_NrM.solve_keep(t, sol, par)
                    #                    negm_HARK.solve_keep(t,sol,par)
                    ...
                    nvfi_NrM.solve_adj(t, sol, par)  # HERE I NEED THE NVFI
    ...
    def simulate(self, do_utility=False, do_euler_error=False):
        ...
        with jit(self) as model:
            ...
            simulate_NrM.lifecycle(sim, sol, par)
        ...
        #        tic = time.time()
        if do_utility:
            simulate_NrM.calc_utility(sim, sol, par)
    ...
    def decision_functions(self):
        figs_NrM.decision_functions(self)

    def egm(self):
        figs_NrM.egm(self)

    def lifecycle(self):
        figs_NrM.lifecycle(self)
