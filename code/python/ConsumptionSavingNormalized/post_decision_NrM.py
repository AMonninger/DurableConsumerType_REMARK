...
def compute_wq(t,sol,par,compute_q=False):
    ...
    for i_n in range(par.Nn):
        ...
        for ishock in range(par.Nshocks):
            ...
            n_plus = ((1 - par.delta) * n) / (psi_plus)
            m_plus = (par.R * par.grid_a)/ psi_plus + xi_plus  # y_plus #R*a-grid + y_plus
            x_plus = m_plus + (1 - par.tau) * n_plus
            ...
            # iv. weight
            # Original weight by Jeppe
            #weight = psi_plus_w*xi_plus_w

            # Corrected one from HARK
            weight = 0.04# *2
            #weight = (psi_plus_w**(1.0 - par.rho)) *xi_plus_w
            ...
            # vi. interpolate
            linear_interp.interp_2d_only_last_vec_mon(prep_keep,par.grid_n,par.grid_m,sol.inv_v_keep[t+1],n_plus,m_plus,inv_v_keep_plus)
            linear_interp.interp_1d_vec_mon(prep_adj,par.grid_x,sol.inv_v_adj[t+1],x_plus,inv_v_adj_plus)
            if compute_q:
                linear_interp.interp_2d_only_last_vec_mon_rep(prep_keep,par.grid_n,par.grid_m,sol.inv_marg_u_keep[t+1],n_plus,m_plus,inv_marg_u_keep_plus)
                linear_interp.interp_1d_vec_mon(prep_adj,par.grid_x,sol.inv_marg_u_adj[t+1],x_plus,inv_marg_u_adj_plus)
