...
def decision_functions(model):
    widgets.interact(
        ...
        name=widgets.Dropdown(
            description="name",
            options=["discrete", "adj", "keep", "post_decision"],
            value="discrete",
        ),
    )
...
def _discrete(model, t):
    ...
    I = inv_v_adj > model.sol.inv_v_keep[t, :, :]
    x = m[I].ravel()
    y = n[I].ravel()
...
def _adj(model, t):
    ...
    # c. plot consumption
    ax_b.plot(par.grid_x, sol.d_adj[t, :], lw=2)
    ax_b.set_title(f"$d^{{adj}}$ ($t = {t}$)", pad=10)

    # d. plot value function
    ax_v.plot(par.grid_x, sol.inv_v_adj[t, :], lw=2)
...
def _w(model, t):
    ...
    # c. plot consumption
    ax.plot_surface(n, a, sol.inv_w[t, :, :], cmap=cm.viridis, edgecolor="none")
    ax.set_title(f"neg. inverse  $w$ ($t = {t}$)", pad=10)
...
def _keep(model, t):
    ...
    # c. plot consumption
    ax_c.plot_surface(n, m, sol.c_keep[t, :, :], cmap=cm.viridis, edgecolor="none")
    ax_c.set_title(f"$c^{{keep}}$ ($t = {t}$)", pad=10)

    # d. plot value function
    ax_v.plot_surface(n, m, sol.inv_v_keep[t, :, :], cmap=cm.viridis, edgecolor="none")
...
def egm(model):
    widgets.interact(
        ...
        model=widgets.fixed(model),
        t=widgets.Dropdown(
            description="t", options=list(range(model.par.T - 1)), value=0
        ),
...
def _egm(model, t, i_n):
    ...
    ax_c.set_title(f"$c$ ($t = {t}$, $n = {par.grid_n[i_n]:.2f}$)", pad=10)

    inv_v_vec = np.zeros(par.Na)
    for i_a in range(par.Na):
        inv_v_vec[i_a] = utility.func(c_vec[i_a], par.grid_n[i_n], par) + (
            -1 / sol.inv_w[t, i_n, i_a]
        )
    inv_v_vec = -1.0 / inv_v_vec

    ax_v.plot(m_vec, inv_v_vec, "o", MarkerSize=0.5, label="before")
    ax_v.set_title(f"neg. inverse $v$ ($t = {t}$, $n = {par.grid_n[i_n]:.2f}$)", pad=10)

    # d. plot after
    c_vec = sol.c_keep[t, i_n, :]
    ax_c.plot(par.grid_m, c_vec, "o", MarkerSize=0.5, label="after")

    inv_v_vec = sol.inv_v_keep[t, i_n, :]
...
def lifecycle(model):
    ...
    for i, (simvar, simvarlatex) in enumerate(simvarlist):
        ...
        if simvar not in ["discrete"]:
            ax.plot(
                age, np.percentile(simdata, 25, axis=1), ls="--", lw=1, color="black"
            )
            ax.plot(
                age, np.percentile(simdata, 75, axis=1), ls="--", lw=1, color="black"
            )
