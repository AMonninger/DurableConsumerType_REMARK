...
class DurableConsumptionModelClass(ModelClass):
    ...
    def solve_prep(self):
        ...
        # b. 2d
        if par.do_2d:
            keep_shape = (par.T, par.Np, par.Nn, par.Nn, par.Nm)
        else:
            keep_shape = (0, 0, 0, 0, 0)
        sol.c_keep_2d = np.zeros(keep_shape)
        sol.inv_v_keep_2d = np.zeros(keep_shape)
        sol.inv_marg_u_keep_2d = np.zeros(keep_shape)

        if par.do_2d:
            adj_full_shape = (par.T, par.Np, par.Nx)
        else:
            adj_full_shape = (0, 0, 0)
        ...
        if par.do_2d:
            adj_part_shape = (par.T, par.Np, par.Nn, par.Nx)
        else:
            adj_part_shape = (0, 0, 0, 0)
        ...
        if par.do_2d:
            post_shape = (par.T - 1, par.Np, par.Nn, par.Nn, par.Na)
        else:
            post_shape = (0, 0, 0, 0, 0)
