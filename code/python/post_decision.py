    ...
    for i_p in prange(par.Np):
        ...
        for i_n in range(par.Nn):
            ...
            for ishock in range(par.Nshocks):
                ...
                prep_keep = linear_interp.interp_3d_prep(
                    par.grid_p, par.grid_n, p_plus, n_plus, par.Na
                )
                ...
                for i_a in range(par.Na):
                    m_plus[i_a] = trans.m_plus_func(
                        par.grid_a[i_a], p_plus, xi_plus, par
                    )
                    x_plus[i_a] = trans.x_plus_func(m_plus[i_a], n_plus, par)

                # vi. interpolate
                linear_interp.interp_3d_only_last_vec_mon(
                    prep_keep,
                    par.grid_p,
                    par.grid_n,
                    par.grid_m,
                ...
                if compute_q:
                    linear_interp.interp_3d_only_last_vec_mon_rep(
                        prep_keep,
                        par.grid_p,
                        par.grid_n,
                        par.grid_m,
                ...
                if compute_q:
                    ...
                    for i_a in range(par.Na):
                        w[i_a] += (
                            ...
                            * (
                                -1.0
                                / np.fmax(inv_v_keep_plus[i_a], inv_v_adj_plus[i_a])
                            )
                        )
